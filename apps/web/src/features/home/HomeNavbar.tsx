  ...
  return (
    <header className="border-b border-[#173452] bg-[#102b49] text-white">
      <div className="mx-auto flex h-[68px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-10">
        <Link to="/" className="flex items-center gap-2" aria-label="GlobeTrotter home">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Main navigation">
          {navigationItems.map((item) => item.href === "/" ? (
            <Link key={item.label} to="/" activeOptions={{ exact: true }} className="rounded-md px-4 py-2 text-[12px] font-medium text-[#b8c8d7] transition-colors hover:bg-white/10 hover:text-white data-[status=active]:bg-[#2c4966] data-[status=active]:text-white">
              {item.label}
            </Link>
          ) : (
            <a key={item.label} href={item.href} className="rounded-md px-4 py-2 text-[12px] font-medium text-[#b8c8d7] transition-colors hover:bg-white/10 hover:text-white">
              {item.label}
            </a>
          ))}
        </nav>
      ...
      {isOpen ? (
        <nav className="border-t border-white/10 px-5 py-3 lg:hidden" aria-label="Mobile navigation">
          {navigationItems.map((item) => (
            <a key={item.label} href={item.href} onClick={() => setIsOpen(false)} className="flex items-center gap-3 border-b border-white/10 px-2 py-3 text-sm text-[#d7e2eb] last:border-0 hover:text-[#f3c969]">
