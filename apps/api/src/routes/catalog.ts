...
export function createCatalogRoutes(dependencies: ApiDependencies) {
  return new Hono<ApiEnvironment>()
    ...
    .get("/cities/:cityId", async (context) => {
    ...
    .get("/activity-categories", async (context) => {
      const rows = await dependencies.database
        .select()
    ...
    .get("/activities", async (context) => {
      ...
      if (query.maxCost !== undefined && query.currency === undefined) {
        throw new DomainError(
          "VALIDATION_ERROR",
          "currency is required when maxCost is supplied.",
          {
      ...
      const categoryId = query.categoryId
        ? parseCatalogId(query.categoryId, "categoryId")
        : undefined
      const cursor = decodeCursor(query.cursor, namedCatalogCursorSchema)
      const cursorId = cursor ? parseCatalogId(cursor.id, "cursor") : undefined
      ...
      return context.json({
        data: page.map(({ activity, category, city }) =>
          serializeActivity(activity, city, category),
        ),
        meta: { nextCursor },
      })
    })

    .get("/activities/:activityId", async (context) => {
