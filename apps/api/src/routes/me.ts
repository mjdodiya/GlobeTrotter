...
export function createMeRoutes(dependencies: ApiDependencies) {
  ...
  routes.get("/deletion-impact", async (context) => {
    const data = await getAccountDeletionImpact(dependencies.database, context.var.session.user.id)
  ...
  routes.get("/calendar.ics", async (context) => {
  ...
  routes.get("/saved-cities", async (context) => {
  ...
  routes.put("/saved-cities/:cityId", async (context) => {
  ...
  routes.delete("/saved-cities/:cityId", async (context) => {
    const cityId = parseCatalogId(context.req.param("cityId"), "cityId")
    await dependencies.database
      .delete(savedCities)
      .where(
