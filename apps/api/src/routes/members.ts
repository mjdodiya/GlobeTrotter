...
export function createMemberRoutes(dependencies: ApiDependencies) {
  return new Hono<ApiEnvironment>()
    .get("/:tripId/members", async (context) => {
    ...
    .post("/:tripId/members", async (context) => {
      const tripId = parseValue(uuidSchema, context.req.param("tripId"))
        ...
        async ({ access, transaction }) => {
          ...
          if (existing) {
            throw new DomainError(
              ...
              "The user is already a trip member.",
            )
          }
    ...
    .delete("/:tripId/members/me", async (context) => {
      const tripId = parseValue(uuidSchema, context.req.param("tripId"))
    ...
    .patch("/:tripId/members/:userId", async (context) => {
      const tripId = parseValue(uuidSchema, context.req.param("tripId"))
    ...
    .delete("/:tripId/members/:userId", async (context) => {
      const tripId = parseValue(uuidSchema, context.req.param("tripId"))
      const targetUserId = memberUserId(context.req.param("userId"))
      const expectedVersion = requireExpectedVersion(context)
